...
const DiscoveryHero = ({ featuredProduct, totalCount = 0, onBrowse }) => {
  ...
  return (
    <section className="relative overflow-hidden border-b border-[#11110E]/10 bg-[#F8F6F1] pt-[82px] text-[#11110E]">
      <div className="mx-auto grid min-h-[calc(100vh-82px)] max-w-[1500px] grid-cols-1 lg:grid-cols-[78px_minmax(0,0.94fr)_minmax(480px,1.06fr)]">
        <aside className="hidden border-x border-[#11110E]/10 lg:flex lg:flex-col lg:items-center lg:justify-end lg:gap-12 lg:pb-20">
          {['Plants', 'Care Tools', 'Pots & Gifts'].map((item) => (
            <span key={item} className="rotate-180 font-label text-[9px] uppercase tracking-[0.34em] text-[#11110E]/58 [writing-mode:vertical-rl]">
              {item}
            </span>
          ))}
        </aside>
        ...
        <div className="flex flex-col justify-center px-6 py-16 md:px-10 lg:px-16 lg:py-20">
          <Motion.p
            ...
            className="font-label text-[9px] uppercase tracking-[0.32em] text-[#6D695F]"
          >
          ...
          <Motion.p
            ...
            className="mt-10 max-w-[470px] font-body text-[15px] leading-8 text-[#4F4B43]"
          >
          ...
          >
            ...
            <div className="flex flex-col justify-center px-4">
              <p className="font-label text-[8px] uppercase tracking-[0.24em] text-[#6D695F]">Shopping Help</p>
              <p className="mt-2 font-body text-[12px] leading-relaxed text-[#4F4B43]">
        ...
        >
          ...
          <div className="absolute bottom-8 right-6 w-[min(320px,calc(100%-48px))] border border-[#11110E]/18 bg-[#FFFEFA]/96 p-5 shadow-[0_24px_70px_rgba(17,17,14,0.16)] backdrop-blur-sm md:right-10">
            <div className="flex items-start justify-between gap-5">
              <p className="font-label text-[8px] uppercase tracking-[0.24em] text-[#6D695F]">{productCode(featuredProduct)}</p>
              <p className="font-label text-[9px] uppercase tracking-[0.16em] text-[#0F3A3A]">{formatPrice(featuredProduct)}</p>
            </div>
            <h2 className="mt-4 font-headline text-[34px] leading-[0.95] text-[#11110E]">{heroName}</h2>
            <p className="mt-3 font-label text-[9px] uppercase tracking-[0.18em] text-[#6D695F]">
              {heroCategory} // {heroSeason}
            </p>
            {heroTags.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-2">
                {heroTags.slice(0, 2).map((tag) => (
                  <span key={tag} className="bg-[#E8E8E0] px-2.5 py-1 font-label text-[8px] uppercase tracking-[0.14em] text-[#4F4B43]">
                    {tag}
                  </span>
                ))}
              </div>
            ...
            {featuredProduct?.id && (
              <Link
                to={`/catalogue/${featuredProduct.id}`}
                className="mt-5 flex w-full items-center justify-center bg-[#11110E] px-5 py-3.5 font-label text-[9px] font-bold uppercase tracking-[0.2em] text-[#FBF9F4] transition-colors hover:bg-[#0F3A3A]"
              >
                View Details
              </Link>
            )}
          ...
          <div className="absolute left-6 top-6 border border-[#11110E]/12 bg-[#FFFEFA]/88 px-4 py-3 backdrop-blur-sm md:left-10">
            <p className="font-label text-[8px] uppercase tracking-[0.22em] text-[#6D695F]">
