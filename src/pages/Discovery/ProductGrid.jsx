  ...
  return (
    ...
    >
      <Link to={`/catalogue/${product.id}`} className="cursor-pointer block">
        ...
        {/* Badge */}
        {(product.badge || product.is_on_sale) && (
          <motion.div
          ...
          >
            {product.is_on_sale && (
              <span className="inline-block bg-[#785A1A] text-white font-label text-[9px] tracking-[0.12em] uppercase px-3 py-1.5 font-medium shadow-lg">
                SALE
              </span>
            )}
            {product.badge && (
      ...
      <Link to={`/product/${product.id}`} className="block">
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex flex-col items-end pt-0.5">
            {product.is_on_sale ? (
              <>
                <span className="font-headline text-[16px] text-[#785A1A] whitespace-nowrap">
                  रू {Number(product.sale_price).toFixed(2)}
                </span>
                <span className="font-body text-[11px] text-[#6B6B6B] line-through opacity-60">
                  {product.price}
                </span>
              </>
            ) : (
